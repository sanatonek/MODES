...
parser.add_argument('--df_baseline', action='store_true')
...
def main():
    ...
    ## Load the dissentangler 
    encoders={M1: encoder_1, M2:encoder_2}
    ...
    rep_disentangler = DeMuReFusion(encoders={M1: encoder_1, M2:encoder_2}, 
                                 ...
                                 mask=True, ckpt_path=args.ckpt_path)  
    ...
    axs[0].set_title("%s masks"%configs[M1]['abv. name'])
    axs[1].set_ylabel('Epochs')
    axs[1].set_title("%s masks"%configs[M2]['abv. name'])
    ...
    baseline_z_test = {M1:encoders[M1](tf.convert_to_tensor(test_batch[M1])), 
                       M2:encoders[M2](tf.convert_to_tensor(test_batch[M2]))}
    baseline_z_valid = {M1:encoders[M1](tf.convert_to_tensor(valid_batch[M1])), 
                       M2:encoders[M2](tf.convert_to_tensor(valid_batch[M2]))}
    if args.df_baseline:                 
    ...
    for data_batch, valid_id_batch in valid_loader:
        ...
        for key in z_s_all.keys():
            ...
            baseline_z_valid[key] = tf.concat([baseline_z_valid[key], encoders[key](tf.convert_to_tensor(data_batch[key]))], 0)
            if args.df_baseline:
                if 'lax' in key:
    ...
    rep_df = pd.DataFrame({'fpath': test_ids, 
                           'zs_%s'%M1:z_s_test[M1].numpy().tolist(),
                           'zs_%s'%M2:z_s_test[M2].numpy().tolist() ,
                           'zm_%s'%M1:z_m_test[M1].numpy().tolist() ,
                           'zm_%s'%M2:z_m_test[M2].numpy().tolist(),
                           'z_s': z_merged_test[:, -zs_size:].numpy().tolist(),
                           'z_baseline_%s'%M1: baseline_z_test[M1].numpy().tolist(),
                           'z_baseline_%s'%M2: baseline_z_test[M2].numpy().tolist(),
                           'z_merged': z_merged_test.numpy().tolist()}) 
    rep_df_valid = pd.DataFrame({'fpath': valid_ids, 
                           'zs_%s'%M1:z_s_valid[M1].numpy().tolist(),
                           'zs_%s'%M2:z_s_valid[M2].numpy().tolist() ,
                           'zm_%s'%M1:z_m_valid[M1].numpy().tolist() ,
                           'zm_%s'%M2:z_m_valid[M2].numpy().tolist(),
                           'z_s': z_merged_valid[:, -zs_size:].numpy().tolist(),
                            'z_baseline_%s'%M1: baseline_z_valid[M1].numpy().tolist(),
                            'z_baseline_%s'%M2: baseline_z_valid[M2].numpy().tolist(),
                            'z_merged': z_merged_valid.numpy().tolist()})
    if args.df_baseline:
        rep_df['z_dropfuse_ecg']=dropfuse_z_test[M1].numpy().tolist()
        rep_df['z_dropfuse_mri']=dropfuse_z_test[M2].numpy().tolist()
        rep_df_valid['z_dropfuse_ecg']=dropfuse_z_valid[M1].numpy().tolist()
        rep_df_valid['z_dropfuse_mri']=dropfuse_z_valid[M2].numpy().tolist()
    ...
    print("\nEvaluating modality-specific %s"%M1)
    s3 = phenotype_predictor(np.vstack(valid_pheno_df['zm_%s'%M1]), valid_pheno_df, 
                             np.vstack(test_pheno_df['zm_%s'%M1]), test_pheno_df,
                             phenotypes=ecg_pheno+mri_pheno+shared_pheno+diagnostics, kfold_indices=kfold_indices,
                             mask=rep_disentangler.modality_masks[M1].binary_mask)
    print("\nEvaluating modality-specific %s"%M2)
    s4 = phenotype_predictor(np.vstack(valid_pheno_df['zm_%s'%M2]), valid_pheno_df, 
                             np.vstack(test_pheno_df['zm_%s'%M2]), test_pheno_df, 
                             phenotypes=ecg_pheno+mri_pheno+shared_pheno+diagnostics, kfold_indices=kfold_indices,
                             mask=rep_disentangler.modality_masks[M2].binary_mask)
    ...
    s5 = phenotype_predictor(np.vstack(valid_pheno_df['z_s']), valid_pheno_df, 
                             ...
                             phenotypes=ecg_pheno+mri_pheno+shared_pheno+diagnostics, kfold_indices=kfold_indices,)
    print("\nEvaluating full %s"%M1)
    f1 = phenotype_predictor(np.concatenate([np.vstack(valid_pheno_df['zm_%s'%M1]),np.vstack(valid_pheno_df['zs_%s'%M1])],-1),
                             valid_pheno_df,
                             np.concatenate([np.vstack(test_pheno_df['zm_%s'%M1]),np.vstack(test_pheno_df['zs_%s'%M1])],-1), 
                             test_pheno_df, 
                             phenotypes=ecg_pheno+mri_pheno+shared_pheno+diagnostics, kfold_indices=kfold_indices,
                             mask=np.concatenate([rep_disentangler.modality_masks[M1].binary_mask, rep_disentangler.shared_mask.binary_mask], 0)) 
    print("\nEvaluating full %s"%M2)
    f2 = phenotype_predictor(np.concatenate([np.vstack(valid_pheno_df['zm_%s'%M2]),np.vstack(valid_pheno_df['zs_%s'%M2])],-1),
                             valid_pheno_df,
                             np.concatenate([np.vstack(test_pheno_df['zm_%s'%M2]),np.vstack(test_pheno_df['zs_%s'%M2])],-1), 
                             test_pheno_df,phenotypes=ecg_pheno+mri_pheno+shared_pheno+diagnostics, kfold_indices=kfold_indices,
                             mask=np.concatenate([rep_disentangler.modality_masks[M2].binary_mask, rep_disentangler.shared_mask.binary_mask], 0))
    print("\nEvaluating baseline %s"%M1)
    b1 = phenotype_predictor(np.vstack(valid_pheno_df['z_baseline_%s'%M1]), valid_pheno_df, 
                             np.vstack(test_pheno_df['z_baseline_%s'%M1]), test_pheno_df, 
                             phenotypes=ecg_pheno+mri_pheno+shared_pheno+diagnostics, kfold_indices=kfold_indices,)
    print("\nEvaluating baseline %s"%M2)
    b2 = phenotype_predictor(np.vstack(valid_pheno_df['z_baseline_%s'%M2]), valid_pheno_df, 
                             np.vstack(test_pheno_df['z_baseline_%s'%M2]), test_pheno_df, 
                             phenotypes=ecg_pheno+mri_pheno+shared_pheno+diagnostics, kfold_indices=kfold_indices,)

    late_fusion = phenotype_predictor_lf(np.vstack(valid_pheno_df['z_baseline_%s'%M1]), valid_pheno_df, 
                                         np.vstack(valid_pheno_df['z_baseline_%s'%M2]), valid_pheno_df, 
                                        phenotypes=ecg_pheno+mri_pheno+shared_pheno+diagnostics, kfold_indices=kfold_indices,)
    if args.df_baseline:
    ...
    m2 = phenotype_predictor(np.concatenate([np.vstack(valid_pheno_df['z_baseline_%s'%M2]),np.vstack(valid_pheno_df['z_baseline_%s'%M1])],-1), 
                             valid_pheno_df, 
                             np.concatenate([np.vstack(test_pheno_df['z_baseline_%s'%M2]),np.vstack(test_pheno_df['z_baseline_%s'%M1])],-1), 
    ...
    plot_pheno_prediction_performance(phenotypes=ecg_pheno+mri_pheno+shared_pheno+diagnostics, 
                                      ...
                                      labels=['%s rep.'%configs[M1]['abv. name'], '%s rep.'%configs[M2]['abv. name'], 'merged rep.', 'concat. re.'],
                                      tag='overall_perf')
    ...
    scores = {'Ours':[np.mean(m1[pheno][1]) for pheno in labels],
            ...
            'Unimodal %s'%configs[M1]['abv. name']:[np.mean(b1[pheno][1]) for pheno in labels],
            'Unimodal %s'%configs[M2]['abv. name']:[np.mean(b2[pheno][1]) for pheno in labels],
            }
    scores_error = {'Ours':[np.std(m1[pheno][1]) for pheno in labels],
                    ...
                    'Unimodal %s'%configs[M1]['abv. name']:[np.std(b1[pheno][1]) for pheno in labels],
                    'Unimodal %s'%configs[M2]['abv. name']:[np.std(b2[pheno][1]) for pheno in labels],
                    }
    ...
    scores = {'%s-specific'%configs[M1]['abv. name']:[np.mean(s3[pheno][1]) for pheno in labels],
              '%s-specific'%configs[M2]['abv. name']:[np.mean(s4[pheno][1]) for pheno in labels],
              'shared':[np.mean(s5[pheno][1]) for pheno in labels],
            }
    scores_error = {'%s-specific'%configs[M1]['abv. name']:[np.std(s3[pheno][1]) for pheno in labels],
              '%s-specific'%configs[M2]['abv. name']:[np.std(s4[pheno][1]) for pheno in labels],
    ...
    for phenotype in ecg_pheno+mri_pheno+shared_pheno+diagnostics:
        ...
        zm_2 = np.vstack(valid_pheno_df['zm_%s'%(M2)])
        mask_2 = rep_disentangler.modality_masks[M2].binary_mask
        zm_2 = np.take(zm_2, np.argwhere(mask_2==1)[:,0], axis=1)
        if zm_2.shape[-1]!=0:
            zm_2 = PCA(n_components=2).fit_transform(zm_2)
            axs[0].scatter(zm_2[:,0], zm_2[:,1], c=valid_pheno_df[phenotype].to_list(),
                            cmap=sns.cubehelix_palette(as_cmap=True))
            axs[0].set_title("Modality-specific (%s)"%configs[M2]['abv. name'], fontsize=14)
        ...
        zm_1 = np.vstack(valid_pheno_df['zm_%s'%M1])
        mask_1 = rep_disentangler.modality_masks[M1].binary_mask
        zm_1 = np.take(zm_1, np.argwhere(mask_1==1)[:,0], axis=1)
        if zm_1.shape[-1]!=0:
            zm_1 = PCA(n_components=2).fit_transform(zm_1)
            axs[2].scatter(zm_1[:,0], zm_1[:,1], c=valid_pheno_df[phenotype].to_list(),
                            cmap=sns.cubehelix_palette(as_cmap=True))
            axs[2].set_title("Modality-specific (%s)"%configs[M1]['abv. name'], fontsize=14)
