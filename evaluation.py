...
def main():
    ...
    _, valid_loader, test_loader, list_ids = load_data(sample_list, data_path, train_ratio=0.6, test_ratio=0.2)
    ...
    print("Evaluating modality-specific MRI")
    ...
    for phenotype in ecg_pheno+mri_pheno:
        labels = ['ecg_zs', 'ecg_zm', 'mri_zs', 'mri_zm', 'pretrained ecg', 'pretrained mri']
        x = np.arange(len(labels))  # the label locations
        width = 0.4  # the width of the bars
        multiplier = 0

        fig, ax = plt.subplots(layout='constrained')
        scores = {'train':(s1[phenotype][0],s3[phenotype][0],s2[phenotype][0],s4[phenotype][0],b1[phenotype][0],b1[phenotype][1]),
         'test':(s1[phenotype][1],s3[phenotype][1],s2[phenotype][1],s4[phenotype][1],b1[phenotype][1],b2[phenotype][1])}
